...
function getDist(num, key) {
    if ( num == 0 ) {
        return (key == 2 || key == 5 || key == 8 || key == 11) 
                    ? Math.abs(11-key) / 3 
                    : Math.abs(15-key) / 3
    } else {
        return (key == 2 || key == 5 || key == 8 || key == 11) 
                    ? Math.abs(num-key) / 3 
                    : parseInt((Math.abs(num-key)+1) / 3) + 1   
    }
}
...
function solution(numbers, hand) {
    ...
    numbers.map(el => {
        if ( el == 1 || el == 4 || el == 7) {
            ...
            const l_dist = getDist(el, l)
            const r_dist = getDist(el, r)
            if (hand == "left") {
                l_dist <= r_dist
                    ? (answer += "L", l = el == 0 ? 11 : el)
                    : (answer += "R", r = el == 0 ? 11 : el)
            } else {
                r_dist > l_dist 
                    ? (answer += "L", l = el == 0 ? 11 : el)
                    : (answer += "R", r = el == 0 ? 11 : el)
